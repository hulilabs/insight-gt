...
function(
    ...
    /**
     * Time to wait between typing for search to be triggered
     * @type {number}
     */
    ...
    var Autocomplete = Vue.extend({
        ...
        methods : {
            ...
            focus : function(shouldTriggerState) {
                // When disabled, avoid focus behavior
                if (this.disabled) {
                    return;
                }
            ...
            _onInputDebounce : function(event) {
                // If the autocomplete is not setup as a search, then
                // proceed directly to input handling (filtering)
                if (!this.search) {
                    this._onInputHandler(event);
                    return;
                }

                // Else: avoid searching on each input, then debounce the search event
