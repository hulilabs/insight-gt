...
function(
    ...
    var Calendar = Vue.extend({
        ...
        props : {
            ...
            /**
             * Object that specifies what days of
             * the month have scheduled events
             */
            daysData : {
                type : Object,
                required : false,
            ...
            /**
             * Toggles a hint on days with events,
             * small circle under the date number
             */
            showBusyDaysHint : {
                type : Boolean,
                required : false,
            ...
            /**
             * Toggles selected week highlight
             */
        ...
        computed : {
            ...
            /**
             * Week number of the selected month's day.
             * Strictly depends on state value's format 'YYYY-MM-DD'
             * @return {number}
             */
            selectedWeekNumber : function() {
                // Avoids unnecessary procesess when no date is selected
                if (!this.state.value) {
                    return null;
                }

                var selectedDate = DateUtil.getUTCValue(this.state.value);
                var selectedDay = selectedDate.getDate();
                var selectedMonth = selectedDate.getMonth();
                var selectedYear = selectedDate.getFullYear();

                var isCurrentMonthVisible = this.currentDateObject.getMonth() === selectedMonth;
                var isCurrentYearhVisible = this.currentDateObject.getFullYear() === selectedYear;

                // True if the calendar of the selected day is visible
                var isSelectedDayVisible = isCurrentMonthVisible && isCurrentYearhVisible;

                // Calculates selected day's week number
                var weekNumber = selectedDay ? Math.ceil((selectedDay + this.currentMonthDaysOffset) / 7) : null;

                return isSelectedDayVisible ? weekNumber : null;
            },
            /**
