...
function(
    ...
    var CustomLogic = Vue.extend({
        ...
        data : function() {
            ...
            return {
                /**
                 * Handle continuos delete keyups
                 * @type {boolean}
                 */
                avoidMultipleDeletions : false,
                /**
                 * State management
                 * @type {Object}
                 */
        ...
        methods : {
             ...
             * @private
             */
            _onInput : function(value, e) {
                var promise = this._updateDataState(value, EVENT.ON_INPUT);

                // Some mobile devices won't trigger a delete keyup, so
                // instead we capture it at input and manually do a delete
                if (e && e.inputType && e.inputType.indexOf('delete') >= 0) {
                    promise.then(this._onDelete);
                }
            ...
            _onDelete : function() {
                ...
                // In order to avoid event handling conflicts between input
                // and keyup.delete, this will fix avoid multiple delete actions
                if (this.avoidMultipleDeletions) {
                    this.avoidMultipleDeletions = false;
                } else {
                    // Get the current value after input and remove the last character if it is an special character
                    ...
                    if (currentValue.indexOf(this.state.previousMaskedValue) === 0 && !MASK_MATCHES.test(lastChar)) {
                        ...
                        this.avoidMultipleDeletions = true;
                        promise = this._updateDataState(value, EVENT.ON_INPUT);
                    }
                }
                return promise;
            },
