...
define([
    ...
    function floodfill(data, x, y, mask, fillcolor, tolerance, width, height) {
        ...
        if (!pixelCompare(i, targetcolor, fillcolor, data, mask, length, tolerance)) { return false; }
        Q.push(i);
        while (Q.length) {
            i = Q.pop();
            if (pixelCompareAndSet(i, targetcolor, fillcolor, data, mask, length, tolerance)) {
                ...
                while (mw < w && mw < (w -= 4) && pixelCompareAndSet(w, targetcolor, fillcolor, data, mask, length, tolerance)); //go left until edge hit
                while (me > e && me > (e += 4) && pixelCompareAndSet(e, targetcolor, fillcolor, data, mask, length, tolerance)); //go right until edge hit
                for (var j = w + 4; j < e; j += 4) {
                    if (j - w2 >= 0 && pixelCompare(j - w2, targetcolor, fillcolor, data, mask, length, tolerance)) Q.push(j - w2); //queue y-1
                    if (j + w2 < length && pixelCompare(j + w2, targetcolor, fillcolor, data, mask, length, tolerance)) Q.push(j + w2); //queue y+1
                }
            }
        }
        return data;
    ...
    function fillUint8ClampedArray(data, x, y, mask, color, tolerance, width, height) {
        ...
        return floodfill(data, xi, yi, mask, color, tolerance, width, height);
    };
    ...
    function fillContext(context, x, y, outlineMask, alphaLevel, tolerance, left, top, right, bottom) {
        var ctx = context;
