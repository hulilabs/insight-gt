  ...
  var Canvas = Vue.extend({
    ...
    methods: {
      ...
      addColor: function(color) {
        ...
        var encondedColor = color.match(/[a-f0-9]{2}/gi);
        this.strokeArray.push(
          encondedColor.map(function(v) {
      ...
      areImagesReady: function() {
        return (
          (!this.hasBaseImage ||
            (this.hasBaseImage && this.baseImageState.loaded)) &&
      ...
      _getPositions: function(e) {
         ...
         */
        var scrollLeft = this._isInsideSeveralScrollableArea(
            DOMUtil.hasHorizontalScroll
          )
            ? DOMUtil.getScrollOffsetLeft(element)
            : 0,
          scrollTop = this._isInsideSeveralScrollableArea(
            DOMUtil.hasVerticalScroll
          )
      ...
      _redrawStroke: function(strokePoints, strokeColor, strokeThickness) {
        ...
        strokeColor = this.isBackgroundBucket
          ? "rgb(200,200,200)"
          : strokeColor;
      ...
      _smoothCanvas: function() {
        var image = this.drawContext.getImageData(
          0,
          0,
          this.width,
          this.height
      ...
      _updateColorArray: function() {
        if (this.hasOutlineImage) {
          var encondedColor = this.currentStrokeColor.match(/[a-f0-9]{2}/gi);
          this.strokeArray[this.state.activeLayer] = encondedColor.map(function(v) {
            return parseInt(v, 16);
          });
